...
export default function IntegrationPage() {
  ...
  const skillMd = useMemo(() => {
    const url = instanceUrl || "https://your-farbencrm-instance.com";
    return `---
name: farbencrm
description: Interact with FarbenCRM — manage workspaces, records, contacts, companies, deals, tasks, notes, and lists.
homepage: ${url}
user-invocable: true
metadata:
  agent:
...
You have access to a FarbenCRM instance. Use its REST API to help users manage their workspaces, contacts, companies, deals, tasks, notes, and lists.

## Configuration

- **Base URL**: \`$FARBENCRM_API_URL\` (e.g. \`${url}\`)
- **Auth**: Bearer token via \`$FARBENCRM_API_KEY\`
- All requests use \`Authorization: Bearer $FARBENCRM_API_KEY\` header
...
`;
    ...
    return JSON.stringify(
      {
        skills: {
          farbencrm: {
            enabled: true,
            env: {
              FARBENCRM_API_URL: url,
              FARBENCRM_API_KEY: selectedKey
  ...
  const installCmd = `mkdir -p ~/.config/farbencrm/skills && cp SKILL.md ~/.config/farbencrm/skills/SKILL.md`;
    ...
    <div className="max-w-3xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Generate a skill file to connect your FarbenCRM instance with an AI
          agent. This lets agents interact with your CRM data.
        </p>
      </div>
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                The URL where your FarbenCRM instance is accessible.
              </p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                The API key your agent will use to authenticate. Create keys in{" "}
                <a
                  href="/settings/api-keys"
                  className="underline hover:text-foreground"
                >
        ...
        <section className="rounded-lg border p-4 space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            After downloading the SKILL.md, install it to your skills directory:
          </p>
          ...
          <div className="rounded-md border border-blue-500/20 bg-blue-500/5 p-3 text-sm space-y-2">
            ...
            <ol className="list-decimal pl-5 space-y-1 text-muted-foreground text-xs">
              ...
              <li>Place it in your agent&apos;s skills directory</li>
              <li>Add the config snippet to your agent config</li>
              <li>Set your actual API key (from Settings &gt; API Keys)</li>
              <li>Restart your agent — it can now interact with your CRM</li>
            </ol>
          </div>
        </section>
      </div>
